...
class BuiltIn
{
    ...
    Action<int, RD.DisplayPipeline>? _setup;
    static Dictionary<uint, DateTime>? _drawTimes;

    private BuiltIn (string name, string datatype, string description, Action<int, RD.DisplayPipeline>? setup)
    {
        Name        = name;
    ...
    public void Setup(uint program, RD.DisplayPipeline dp)
    {
        if (_setup == null)
            return;
        string glname = Name;
    ...
    static ON.Light[]? _preSR3Lights;
    static ON.Light[] GetLightsHelper(RD.DisplayPipeline pipeline)
    {
        ...
        if (method == null)
        {
            if( _preSR3Lights==null )
            {
                ...
                var light = new ON.Light {
                    LightStyle      = ON.LightStyle.CameraDirectional,
                    Direction       = new ON.Vector3d(1, -1, -3),
                    ShadowIntensity = 0.7
                };
                _preSR3Lights = new ON.Light[] { light };
            }
            return _preSR3Lights;
        }
        var lights = method.Invoke(pipeline, null) as ON.Light[];
        return lights!;
    }
    ...
    public static List <BuiltIn> GetUniformBuiltIns ()
    {
        if( _uniformBuiltins==null)
        {
            ...
            {
                var xf = display.Viewport.GetTransform(RH.DocObjects.CoordinateSystem.World, RH.DocObjects.CoordinateSystem.Camera);
                xf = xf.Transpose();
                ON.Transform m;
            ...
            {
                var m = display.Viewport.GetTransform(RH.DocObjects.CoordinateSystem.World, RH.DocObjects.CoordinateSystem.Screen);
                m = m.Transpose();
            ...
            Register($"_lightInCameraSpace[{maxlights}]", "vec3", "array of four light posiitons in camera space", (location, display) =>
            {
                ...
                for (int i = 0; i < lights.Length; i++)
                {
                    ...
                    bool isCameraLight = lights[i].CoordinateSystem == RH.DocObjects.CoordinateSystem.Camera;
    ...
    public static List<BuiltIn> GetAttributeBuiltIns()
    {
        return new List <BuiltIn> {
            new ("_meshVertex", "vec3", "mesh vertex location", null),
            new ("_meshNormal", "vec3", "mesh normal", null),
            new ("_meshTextureCoordinate", "vec2", "mesh texture coordinate", null),
            new ("_meshVertexColor", "vec4", "mesh vertex color", null)
        };
    }

    static void Register (string name, string datatype, string description, Action<int, RD.DisplayPipeline> setup)
    {
        if (_uniformBuiltins == null)
